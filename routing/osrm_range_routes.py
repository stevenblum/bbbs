...
# ---- Config (edit these) ----
CSV_PATH = Path("data_geocode/latest/data_geocode.csv")
START_DATE = "2021-06-01"  # YYYY-MM-DD; leave blank to auto-pick range
END_DATE = "2021-07-01"    # YYYY-MM-DD; leave blank to auto-pick range
DEFAULT_RANGE_DAYS = 30
OUTPUT_HTML = Path(__file__).with_name("osrm_range_routes_map.html")
  ...
  <script>
    ...
    function renderLegend(day) {{
      ...
      const rows = day.routes.map((route) => {{
        const osrmTag = route.osrm_error ? ' (OSRM fallback)' : '';
        return (
