# Geocoding
## Todo
Update viz route map so it uses the new savers CSV file name
...
Refactoring Data Processing
  Format the routes with the depot earlier
  Have a clear database like dependency plan
  Reduce duplication in many areas
...
Now that we have geocoded stop level data, we need to create a script that organizes the data into a stadard format that will be good for an optimizer to solve. Inside the optimize folder, create a script called create_problem_instances.py. At the stop, this script should have a constant variables that are START_DATE and END_DATE, which will be the range that the problem instance will be created for. The problem instance should also be executable from the command line with these constandts as arguments, so please use argparse to impelemnt that. I expect the date range to frequently just be one day, and at most possibly a week, it would almost never be a month. Then, the script will need to load data_geocode.csv, and select all of the stops that appear in that date range. Then it needs to load the data_bins.csv and data_routine.csv and transform, it will need to look at all of the display names in the data range, and then look through these csvs, and change any "Other" display names into the primary dispaly names so all bins and routine are mapped to the correct locations. Then find the unique dispaly names that are in the date range and make a column in the dataframe that includes how many times they were visited in the date range. Now, we are going to need to go back through teh data_bins.csv and data_routine.csv add the stops_in_previous_30 and stops_in_previous_7 features. Also, if there any active bins or routing that are not in the range, add them, make the number of visits in the range 0, but still add their features. From  this point forward, the order of teh unique display names can not be changed because we are going to create a matrix that represents the travel time between stops. So, we are going to use OSRM to get the travel time between stops, so get the latitude and longitude for each dispaly name, and pass it to osrm in such a way that it will pass back distances. Now we need to build the output csv file, problem_instance_YYYY_MM_DD.csv. It should be a json file, where the top is the list of stops in the defined order with all of the features, and then the bottom is a travel time matrix that represents the travel time between each stop.

# Optimize with Google OR Tools
Now that we have the problem instance file, we need to create a script that can read in the problem instance file, and then use the Google OR Tools to solve the problem. Inside the optimize folder, create a script called solve_problem_ortools.py. This script should take in a problem instance file as an argument, and then read in the data. Then it should use the Google OR Tools to solve the problem, and output a solution csv file that is structures so exactly like the existing @viz_map_routes_on_road.py can plot it. Look specifically at what columns @viz_map_routes_on_road.py take from the @data_geocoded.csv and add these to the output file. THen also, as part of this solving script, run the @viz_map_routes_on_road.py with it pointed at the solution csv.
